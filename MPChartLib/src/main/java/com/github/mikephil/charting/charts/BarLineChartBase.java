...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    private boolean mDragCoordEnable = false; // add by yuzh 是否可以拖动坐标
...
//            Log.i(LOG_TAG, "Drawtime: " + drawtime + " ms, average: " + average + " ms, cycles: " + drawCycles);
    ...
    public void calculateOffsets() {
        ...
        if (!mCustomViewPortEnabled) {
            ...
            if (mXAxis.isEnabled() && mXAxis.isDrawLabelsEnabled()) {
                ...
                float xLabelHeight = mXAxis.mLabelRotatedHeight + mXAxis.getYOffset() + 20.f; // add by yuzh (+ 20.f) 防止label贴在底部了

                // offsets for x-labels
                if (mXAxis.getPosition() == XAxisPosition.BOTTOM) {

                    offsetTop += xLabelHeight;    // add by yuzh
    ...
    // add by yuzh
    public void setDragCoordEnable(boolean enable) {
        this.mDragCoordEnable = enable;
    }

    public boolean isDragCoordEnable() {
        return mDragCoordEnable;
    }
