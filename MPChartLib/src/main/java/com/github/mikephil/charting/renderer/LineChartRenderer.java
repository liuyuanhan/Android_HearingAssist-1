...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public void drawValues(Canvas c) {
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                float yAxisMinimum = mChart.getAxis(YAxis.AxisDependency.LEFT).getAxisMinimum();

                float[] positions = !dataSet.isDrawValuesTop() // add by yuzh
                        ? trans.generateTransformedValuesLine(dataSet, mAnimator.getPhaseX(), mAnimator.getPhaseY(), mXBounds.min, mXBounds.max)
                        : trans.generateTransformedValuesLine2(dataSet, yAxisMinimum, mAnimator.getPhaseX(), mAnimator.getPhaseY(), mXBounds.min, mXBounds.max);
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (dataSet.isDrawValuesEnabled()) {
                        // change by yuzh 可以绘制不同的ValueSize
                        if (dataSet.getValueTextSizes() == null) { drawValue(c, dataSet.getValueFormatter(), entry.getY(), entry, i, x,y - valOffset, dataSet.getValueTextColor(j / 2)); }
                        else { drawValue(c, dataSet.getValueFormatter(), entry.getY(), entry, i, x,y - valOffset, dataSet.getValueTextColor(j / 2), dataSet.getValueTextSize(j / 2)); }
                    }
    ...
    private class DataSetImageCache {
         ...
         * 在画布上生成一个临时的bitmaps,然后将这个bitmap绘制出来即可
         * @param set
        ...
        protected void fill(ILineDataSet set, boolean drawCircleHole, boolean drawTransparentCircleHole) {
            ...
            boolean drawCross = set.isDrawCrossEnable(); // add by yuzh 是否画叉
            ...
            for (int i = 0; i < colorCount; i++) {
                ...
                if (drawTransparentCircleHole) {
                    ...
                    if (drawCross) {
                        mRenderPaint.setStrokeWidth(4);
                        canvas.drawLine(0, 0, circleRadius*2, circleRadius*2, mRenderPaint);
                        canvas.drawLine(circleRadius*2, 0, 0, circleRadius*2, mRenderPaint);
                    } else {
                        canvas.drawCircle(
                                circleRadius,
                                circleRadius,
