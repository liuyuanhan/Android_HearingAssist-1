...
public abstract class MediaEncoder implements Runnable {
    ...
    public MediaEncoder(final MediaMuxerWrapper muxer, final MediaEncoderListener listener) {
		...
		mListener = listener;
        synchronized (mSync) {
            mBufferInfo = new MediaCodec.BufferInfo(); // create BufferInfo here for effectiveness(to reduce GC)
            new Thread(this, getClass().getSimpleName()).start(); // wait for starting thread
            try {
            	mSync.wait();
...
//					mBufferInfo.size = 0; // 写入muxer 可以忽悠sps pps
                }
                ...
                if (mBufferInfo.size != 0) {
            		...
            		count = 0;
                    if (!mMuxerStarted) {
                        throw new RuntimeException("drain:muxer hasn't started"); // muxer is not ready...this will prrograming failure.
                    }
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) { // when EOS come; out of while
                	Log.d(TAG, "drain: EOS Come, out of while!");
