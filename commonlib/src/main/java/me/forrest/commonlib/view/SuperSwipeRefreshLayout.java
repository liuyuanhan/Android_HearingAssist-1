...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    private int mCurrentTargetOffsetTop; // -mHeaderViewHeight
    private boolean mOriginalOffsetCalculated = false;

    private float mInitialMotionY; // 记录按下的位置
    ...
    public SuperSwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mSpinnerFinalOffset = DEFAULT_CIRCLE_TARGET * metrics.density; // 最后停顿时的偏移量px，与DEFAULT_CIRCLE_TARGET正比
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (usingDefaultHeader) {
                        ...
                        defaultProgressView.setScaleX(alpha);
                        defaultProgressView.setScaleY(alpha);
                        defaultProgressView.setAlpha(alpha);
                    }
                    if (overscrollTop < mTotalDragDistance) {
                        if (mScale) { setAnimationProgress(overscrollTop / mTotalDragDistance); }
                        if (mListener != null) { mListener.onPullEnable(false); }
                    } else {
